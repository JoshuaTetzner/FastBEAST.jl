...
Γ = CompScienceMeshes.read_gmsh_mesh(pwd()*"/U/geo/U_"*ARGS[1]*".msh")
...
results = results * "U EFIE; pivoting: MaxPivoting, convergence: Standard, h: " * ARGS[1]
results = results * "\nN \t storage in GB \t compression \t" * "rel matrix error\n" 
#---------------------------------------
# Write data
#---------------------------------------
file = open(pwd() * "/U/results_UEFIE.txt", "r")
oldresults = read(file, String)
close(file)
file = open(pwd() * "/U/results_UEFIE.txt", "w")
