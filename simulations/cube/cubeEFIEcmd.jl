...
results = results * "cube EFIE; pivoting: MaxPivoting, convergence: Standard, h: " * ARGS[1]
results = results * "\nN \t storage in GB \t compression \t" * "rel matrix error\n" 
#---------------------------------------
# Write data
#---------------------------------------
file = open(pwd() * "/cube/results_cubeEFIE.txt", "r")
oldresults = read(file, String)
close(file)
file = open(pwd() * "/cube/results_cubeEFIE.txt", "w")
