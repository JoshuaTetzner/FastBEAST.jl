...
Γ = CompScienceMeshes.meshcuboid(1.0, 1.0, 1.0, 0.1)
...
K_bc = hassemble(
    ...
    compressor=FastBEAST.ACAOptions(
        rowpivstrat=piv, convcrit=conv, maxrank=500, tol=tol
    ),    
    verbose=true,
    multithreading=true
)
...
results = results * "cube; pivoting: " * ARGS[1] * ", convergence: " * ARGS[2] * ", tol: " * ARGS[3] 
results = results * "\nN \t storage \t compression in GB \t" *  
    "rel matrix error\n" 
#---------------------------------------
# Write data
#---------------------------------------
file = open(pwd() * "/cube/results_cubesweep.txt", "r")
oldresults = read(file, String)
close(file)
file = open(pwd() * "/cube/results_cubesweep.txt", "w")
